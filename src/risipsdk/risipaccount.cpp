...
QQmlListProperty<RisipBuddy> RisipAccount::buddies()
{
    if(m_data->pjsipAccount) {
       ...
       // Replaced with BuddyVector2 as BuddyVector depricated

        BuddyVector2 buddies = m_data->pjsipAccount->enumBuddies2();
        RisipBuddy *buddy;
        for(int i=0; i<m_data->allBuddies.count(); ++i) {
            buddy = new RisipBuddy(this);
            buddy->setAccount(this);
         // [SG]  buddy->setPjsipBuddy(static_cast<PjsipBuddy *>(buddies[i]));
...
RisipBuddy *RisipAccount::findBuddy(const QString &uri)
{
    ...
    if(!uri.isEmpty()) {
       ...
       //[TBD]
            //PjsipBuddy *pjsipBuddy = static_cast<PjsipBuddy *>(m_data->pjsipAccount->findBuddy(findBuddy(uri.toStdString()));
            PjsipBuddy *pjsipBuddy;
            Buddy z  = m_data->pjsipAccount->findBuddy2(uri.toStdString());
            pjsipBuddy = static_cast<PjsipBuddy *>(&z);
            if(pjsipBuddy) {
